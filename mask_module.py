...
class MaskCreator:
    ...
    # Marking craters rims with distortions
    def _mark_crater_rim_dis(self, longitude, latitude, long_limit, lat_limit, radius_km):
    ...
    def place_craters_dis(self, csv_dir, bounds):
        ...
        for index, row in df.iterrows():
            ...
            if bounds[2] < longitude < bounds[3] and bounds[0] < latitude < bounds[1]:
                radius = row['DIAM_CIRC_IMG'] / 2
                self._mark_crater_rim_dis(longitude, latitude, bounds[2], bounds[1], radius)
            else:
                rejected_craters_counter += 1
            # Update process status
            if int(index / num_rows * 100) > process_counter:
