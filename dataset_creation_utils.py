...
from config import CONST_PATH, INPUT_ZIP_NAME, TILES_NAMES
from config import CRATERS_CATALOGUE_NAME
from config import FIRST_COL_ID, CSV_TILES_NAMES, COLS_NAMES_TO_ANALYZE
from config import SCALE_KM, MEAN_MOON_RADIUS_KM, LONGITUDE_MOON_CIRCUMFERENCE_KM, CRATER_RIM_INTENSITY, \
                   CSV_TILES_KEYS, TILES_BOUNDS
from config import MIN_CROP_AREA_SIZE_KM, MAX_CROP_AREA_SIZE_KM, SCALE_PX, SAMPLE_RESOLUTION
...
def create_dataset(no_samples_train=0, no_samples_valid=0, no_samples_test=0):
    # Check if working dirs exists, if not create it
    print("\nCHECKING DIRECTORIES:\n")
    dir_module()
